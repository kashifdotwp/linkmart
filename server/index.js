...
app.post('/api/send-email', async (req, res) => {
  ...
  // Build HTML version with proper structure
  ...
  try {
    ...
    const info = await transporter.sendMail({
      ...
      headers: {
        ...
        'X-Mailer': 'LinkMart Outreach v2.0',
...
// ─── Start Server ────────────────────────────────────────────────────────────
app.listen(PORT, () => {
  console.log(`\n✅ LinkMart Backend Server running on http://localhost:${PORT}`);
  console.log(`   Email API: POST /api/send-email`);
  console.log(`   AI API:    POST /api/generate-email`);
